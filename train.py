...
_CAMERA = flags.DEFINE_string(
    "camera",
    "track",
    "Camera name to use for rendering (e.g., track, top, side, back).",
)
...
def main(argv):
    ...
    if _SUFFIX.value is not None:
        exp_name = f"{timestamp}-{_SUFFIX.value}"
    else:
        exp_name = timestamp
    print(f"Experiment name: {_ENV_NAME.value}/{exp_name}")

    # Set up logging directory: logs/<env_name>/<timestamp>
    logdir = epath.Path("logs").resolve() / _ENV_NAME.value / exp_name
    ...
    if _USE_WANDB.value and not _PLAY_ONLY.value:
        ...
        wandb.init(project="mjxrl", name=exp_name, dir=str(logdir))
    ...
    total_timesteps = ppo_params.num_timesteps  # 保存总步数
    ...
    def progress(num_steps, metrics):
        ...
        # 计算并显示进度
        progress_pct = num_steps / total_timesteps * 100 if total_timesteps > 0 else 0
        elapsed = times[-1] - times[1] if len(times) > 1 else 0
        steps_per_sec = num_steps / elapsed if elapsed > 0 else 0
        remaining_steps = total_timesteps - num_steps
        ...
        if _RUN_EVALS.value:
            if "eval/episode_reward" in metrics:
                print(
                    ...
                    f"reward={metrics['eval/episode_reward']:.3f} | "
                    f"ETA: {eta/60:.1f}min"
                )
    ...
    # Create video directory under logdir
    ...
    for i, rollout in enumerate(trajectories):
        ...
        frames = eval_env.render(
            traj, camera=_CAMERA.value, height=480, width=640, scene_option=scene_option
        )
